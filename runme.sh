...
: ${MODULES:=at24,authenc,bluetooth,btbcm,btintel,btrtl,btusb,can_dev,cdns3,cdns3_ti,cfg80211,crct10dif_ce,ecc,ecdh_generic,fuse,hdc2010,icss_iep,icssg_prueth,ip_tables,ipv6,irq_pruss_intc,iwlwifi,m_can,m_can_platform,opt3001,pci_endpoint_test,pru_rproc,pruss,qcserial,rfkill,rtc_abx80x,rti_wdt,sa2ul,sha512_generic,ti_k3_r5_remoteproc,udc_core,usb_storage,usb_wwan,usbcore,x_tables,usbserial,xhci_hcd,xhci_plat_hcd}
...
make CROSS_COMPILE=arm-linux-gnueabihf- SOC=$SOC SOC_TYPE=$SOC_TYPE clean
make CROSS_COMPILE=arm-linux-gnueabihf- SOC=$SOC SOC_TYPE=$SOC_TYPE
